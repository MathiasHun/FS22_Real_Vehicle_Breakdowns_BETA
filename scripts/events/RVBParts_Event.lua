...
function RVBParts_Event:readStream(streamId, connection)
	...
	for i=1, #self.vehicle.spec_faultData.parts do
		...
		self.vehicle.spec_faultData.parts[i].lifeage = streamReadInt16(streamId)
		self.vehicle.spec_faultData.parts[i].age = streamReadInt16(streamId)
...
function RVBParts_Event:writeStream(streamId, connection)
	...
	for i=1, #self.vehicle.spec_faultData.parts do
		...
		streamWriteInt16(streamId, self.vehicle.spec_faultData.parts[i].lifeage)
		streamWriteInt16(streamId, self.vehicle.spec_faultData.parts[i].age)
